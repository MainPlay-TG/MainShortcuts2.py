...
class PlatformInfo(ms.ObjectBase):
  ...
  def _mkdir(self, path):
    return ms.dir.create(path, _exists=self._created_dirs)

  @cached_property
  def is_windows(self):
    """Use .is_mustdie instead"""
    return self.is_mustdie

  @cached_property
  def user_desktop_dir(self):
    return self._mkdir(os.environ.get("XDG_DESKTOP_DIR", self.home / "Desktop"))
...
class PlatformLinux(_Platform):
  ...
  def system_bin_dir(self):
    return self._mkdir(self.root_dir / "usr/local/bin")

  @cached_property
  def system_cache_dir(self):
    return self._mkdir(self.root_dir / "var/cache")

  @cached_property
  def system_config_dir(self):
    return self._mkdir(self.root_dir / "etc")

  @cached_property
  def system_data_dir(self):
    return self._mkdir(self.root_dir / "usr/share")

  @cached_property
  def system_lib_dir(self):
    return self._mkdir(self.root_dir / "lib")

  @cached_property
  def system_log_dir(self):
    return self._mkdir(self.root_dir / "var/log")

  @cached_property
  def user_bin_dir(self):
    return self._mkdir(self.home / ".local/bin")

  @cached_property
  def user_cache_dir(self):
    return self._mkdir(self.home / ".cache")

  @cached_property
  def user_config_dir(self):
    return self._mkdir(self.home / ".config")

  @cached_property
  def user_data_dir(self):
    return self._mkdir(self.home / ".local/share")

  @cached_property
  def user_lib_dir(self):
    return self._mkdir(self.home / ".local/lib")

  @cached_property
  def user_log_dir(self):
    return self._mkdir(self.home / ".local/log")
...
class PlatformMacOS(_Platform):
  ...
  @cached_property
  def system_bin_dir(self):
    return self._mkdir(self.root_dir / "usr/bin")

  @cached_property
  def system_cache_dir(self):
    return self._mkdir(self.root_dir / "private/var/cache")

  @cached_property
  def system_config_dir(self):
    return self._mkdir(self.root_dir / "private/etc")
  ...
  @cached_property
  def system_lib_dir(self):
    return self._mkdir(self.root_dir / "usr/lib")

  @cached_property
  def system_log_dir(self):
    return self._mkdir(self.root_dir / "private/var/log")

  @cached_property
  def user_bin_dir(self):
    return self._mkdir(self.home / "bin")

  @cached_property
  def user_cache_dir(self):
    return self._mkdir(self.home / "Library/Caches")

  @cached_property
  def user_config_dir(self):
    return self._mkdir(self.home / "Library/Preferences")

  @cached_property
  def user_data_dir(self):
    return self._mkdir(self.home / "Library/Application Support")
  ...
  @cached_property
  def user_log_dir(self):
    return self._mkdir(self.home / "Library/Logs")
...
class PlatformMustdie(_Platform):
  @cached_property
  def root_dir(self):
  ...
  @cached_property
  def system_cache_dir(self):
    return self._mkdir(self.root_dir / "Windows/Temp")

  @cached_property
  def system_config_dir(self):
    return self._mkdir(self.root_dir / "Windows/System32/config")

  @cached_property
  def system_data_dir(self):
    return self._mkdir(self.root_dir / "ProgramData")
  ...
  @cached_property
  def system_log_dir(self):
    return self._mkdir(self.root_dir / "Windows/System32/LogFiles")

  @cached_property
  def user_bin_dir(self):
    return self._mkdir(self.user_data_dir / "Microsoft/WindowsApps")

  @cached_property
  def user_cache_dir(self):
    return self._mkdir(self.user_data_dir / "Temp")

  @cached_property
  def user_config_dir(self):
    return self._mkdir(self.home / "AppData/Roaming")

  @cached_property
  def user_data_dir(self):
    return self._mkdir(self.home / "AppData/Local")

  @cached_property
  def user_lib_dir(self):
    return self._mkdir(self.user_data_dir / "Microsoft/Windows/Libraries")

  @cached_property
  def user_log_dir(self):
    return self._mkdir(self.user_data_dir / "Microsoft/Windows/Logs")
  ...
  @cached_property
  def user_desktop_dir(self):
    return self._mkdir(self.home / "Desktop")
  ...
  def sleep(self):
    self._run("rundll32", "powrprof.dll,SetSuspendState", "0,1,0")


PlatformWindows = PlatformMustdie
...
class _NetBroadcastSenderBase(_ServerBase):
  ...
  @classmethod
  def simple_send(cls, port: int, data: bytes, **kw):
    """Просто отправить сообщение 1 раз"""
    with cls(port, data, **kw) as self:
      self.send()


class NetBroadcastSender(_NetBroadcastSenderBase):
  ...
  def __init__(self, port: int, data: bytes, ipv6=False):
    ...
    _NetBroadcastSenderBase.__init__(self, port, data)
    self._ipv6 = ipv6
...
class NetBroadcastSenderMultiIPv4(_NetBroadcastSenderBase):
  def __init__(self, port: int, data: bytes, addrs: set[str] = None, remove_errored=True):
    from socket import socket, AF_INET, SO_BROADCAST, SOCK_DGRAM, SOL_SOCKET
    _NetBroadcastSenderBase.__init__(self, port, data)
    self.addrs: set[str] = set()
    self.remove_errored = remove_errored
      ...
      for addr in addrs:
        if addr.broadcast:
          if addr.family == AF_INET:
            self.addrs.add(addr.broadcast)

  def send(self, data=None):
    data = self.data if data is None else self._conv_data(data)
    for ip in self.addrs:
      try:
        self._sock.sendto(data, (ip, self.port))
      except Exception as exc:
        self.on_error(ip, exc)
        if self.remove_errored:
          self.addrs.remove(ip)

  def on_error(self, ip: str, exc: Exception):
    pass
